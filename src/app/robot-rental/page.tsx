...
type Copy = {
  ...
  lineup: { name: string; type: string; desc: string }[];
...
const COPY: Record<"ja" | "en", Copy> = {
  ja: {
    ...
    lineup: [
      { name: "Unitree G1", type: "ヒューマノイド", desc: "コンパクトな人型ロボット。研究・実証・展示など幅広い用途を想定。" },
      { name: "Unitree H1", type: "ヒューマノイド", desc: "高い運動性能を持つ大型人型ロボット。先進的なデモ・実証向け。" },
      { name: "Unitree Go2", type: "四足歩行ロボット", desc: "機動性の高い四足歩行型。点検・巡回・イベント演出などに。" },
  ...
  en: {
    ...
    lineup: [
      { name: "Unitree G1", type: "Humanoid", desc: "A compact humanoid robot, intended for research, PoC, and exhibitions." },
      { name: "Unitree H1", type: "Humanoid", desc: "A larger humanoid with high mobility — for advanced demos and proof of concept." },
      { name: "Unitree Go2", type: "Quadruped", desc: "A highly agile quadruped — for inspection, patrol, and event production." },
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {t.lineup.map((item, i) => (
              <Reveal key={item.name} delay={i * 80}>
                <div className="relative h-full rounded-2xl border border-gray-200 bg-white p-6 hover:shadow-lg transition-all duration-300">
                  <span className="absolute top-4 right-4 inline-flex items-center rounded-full bg-gray-100 text-gray-500 px-2.5 py-0.5 text-[10px] font-bold tracking-wide">
                    {t.comingSoon}
                  </span>
