    ...
    <>
      ...
      <div className="bg-white">
        <PageHeader
          kicker="Case Studies"
          title="導入実績"
          description="実際にお受けした案件を、課題・つくったもの・結果の順で記録しています。うまくいった部分だけでなく、あえて自動化しなかった範囲や、お断りした要望も書いています。ご検討中の内容に近いものがあれば、そのままご相談の材料としてお使いください。"
          crumbs={[{ label: "ホーム", href: "/" }, { label: "導入実績" }]}
          actions={
                ...
                <li key={c.slug}>
                  ...
                  >
                    ...
                    <div className="flex shrink-0 items-center gap-3 md:w-48 md:flex-col md:items-start md:gap-2">
                      ...
                      <span className="text-xs font-medium text-neutral-900">{c.category}</span>
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="mb-2 text-xs text-neutral-500">
                        {c.industry}／{c.companySize}
                      </p>
                      ...
                      <p className="line-clamp-3 text-sm leading-[1.9] text-neutral-600">{c.summary}</p>
                    </div>
                  </Link>
                </li>
              ))}
        ...
        <section className="border-t border-neutral-200 bg-neutral-950 py-20 md:py-28">
          <div className="max-w-[1800px] mx-auto px-6 lg:px-8 text-center">
            <h2 className="mb-5 text-2xl md:text-3xl font-bold leading-snug text-white">
              近い事例があれば、そのまま持ち込んでください。
            </h2>
            <p className="mb-10 text-[15px] leading-[1.9] text-white/60">
              「この事例のこの部分を自社でもやりたい」というご相談が一番進みが速いです。
              実現できない範囲もその場でお伝えします。初回のご相談は無料です。
            </p>
            <Link
              href="/contact"
