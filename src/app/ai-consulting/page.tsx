...
const COPY: Record<"ja" | "en", Copy> = {
  ...
  en: {
    ...
    services: [
      { num: "01", title: "AI strategy", desc: "From current-state analysis to roadmap development. We calculate ROI and draft an actionable plan that starts with the highest-priority areas.", hoverBg: "hover:bg-blue-50" },
  ...
  return (
    <>
      ...
      <PageHero
        ...
        primaryHref="/contact"
        secondaryLabel={lang === "ja" ? "支援領域を見る" : "View services"}
        secondaryHref="#services"
        accent="blue"
        scrollLabel={lang === "ja" ? "Scroll" : "Scroll"}
      />
            ...
            {t.plans.map((plan, i) => (
              <Reveal key={plan.name} delay={i * 100} className="flex">
                <div className={`rounded-2xl p-8 lg:p-10 transition-all duration-300 flex flex-col w-full ${plan.featured ? "bg-blue-600 text-white shadow-xl" : "bg-white border border-gray-200 hover:shadow-lg"}`}>
                  ...
                  <a href="/contact" className={`block text-center text-sm font-semibold py-3 rounded-lg transition-all duration-300 mt-auto ${plan.featured ? "bg-white text-blue-600 hover:bg-blue-50" : "border border-gray-200 text-gray-600 hover:border-gray-400 hover:text-gray-900"}`}>{t.planCta}</a>
                </div>
              </Reveal>
            ))}
          </div>
