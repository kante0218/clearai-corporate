...
/*
 * カード一覧。PC(md+)は gridClass の従来グリッド、SPは縦積み1カラム。
 * 2026-08-11ユーザー指示: SPの横スワイプ(カルーセル)は「横にムニムニ動く」ため廃止。
 * ドットとスクロール追従のコードは互換のため残しているが、SPでは横スクロールが
 * 発生しないので実質何もしない。children は 1枚=1カード。
 */
export default function CardCarousel({ children, className = "", gridClass = "md:grid-cols-3" }: { children: ReactNode; className?: string; gridClass?: string }) {
  ...
  return (
    <div className={className}>
      <div
        ref={trackRef}
        className={`grid grid-cols-1 ${gridClass} gap-4 md:gap-6 items-stretch`}
      >
        {items.map((child, i) => (
          <div key={i} data-cc-card className="w-full flex [&>*]:w-full">{child}</div>
        ))}
      </div>
      {items.length > 1 && (
