...
改善前の7日 baseline を取得できれば保存し、取得不能なら理由付き null とする。リポジトリの既存検証と本番反映手順を実施。コードを置いただけで観察開始にせず、本番URLで変更を確認して deployedAt を確定する。デプロイ失敗は状態と未完了事項に明記する。現行 clearai-corporate は vercel.json で main の自動デプロイが無効。ページ変更は build・独立レビュー・commit・pull --rebase・push 後、リンク済みリポジトリで `vercel --prod --yes` を実行し、Ready と本番HTMLを検証する。観察記録だけのコミットに再デプロイは不要。
